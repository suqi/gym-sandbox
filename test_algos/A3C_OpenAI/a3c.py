        ...
        for _ in range(num_local_steps):
            ...
            if info:
                ...
                for k, v in info.items():
                    try:
                        summary.value.add(tag='info/{}'.format(k), simple_value=float(v))
                    except TypeError:
                        pass
            ...
            if terminal:
                terminal_end = True
                if not env.metadata.get('semantics.autoreset'):
                    last_state = env.reset()
...
"""
        ...
        with tf.device(tf.train.replica_device_setter(1, worker_device=worker_device)):
            with tf.variable_scope("global"):
                self.network = QiduPolicy(env.observation_space.shape, env.action_space.n)
                self.global_step = tf.get_variable("global_step", [], tf.int32, initializer=tf.constant_initializer(0, dtype=tf.int32),
                                                   trainable=False)

        with tf.device(worker_device):
            with tf.variable_scope("local"):
                self.local_network = pi = QiduPolicy(env.observation_space.shape, env.action_space.n)
            ...
            if use_tf12_api:
                ...
                # tf.summary.image("model/police", pi.x[:,:,:,0])
                # tf.summary.image("model/thief", pi.x[:,:,:,1])
