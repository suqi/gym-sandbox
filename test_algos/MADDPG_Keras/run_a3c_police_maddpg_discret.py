...
env = gym.make(GAME)
_s = env.reset()
N_S = _s[0].shape[0]  # env.observation_space.shape[0]
...
class Worker(object):
    def __init__(self, name, globalAC):
        ...
        if name == 'W_0':
            self.env.env.init_params(show_dashboard=True, bokeh_output='standalone')
        self.name = name
            ...
            while True:
                ...
                if RUN_MODE == "training" and (total_step % UPDATE_GLOBAL_ITER == 0 or done):  # update global and assign to local net
                    if done:
                        ...
                        v_s_ = SESS.run(self.AC.v, {self.AC.s: s_[0][np.newaxis, :]})[0, 0]
                    buffer_v_target = []
